...
def log_orthogonal_views_to_tb(writer, global_step, input_extra_t, input_t, fake_t, target_t, spacing=(0.0362, 0.2, 0.2), save_dir=None):
    """
    核心监控函数：截取 3D 矩阵正中心切片，根据物理间距修正长宽比，绘制 3x4 对比图并送入 TensorBoard。
    布局：
        行: XY断面 (横切), ZY断面 (矢状), ZX断面 (冠状)
        列: Extra (极左), Input (中左), Fake (中右), Target (极右)
    """
    dz, dy, dx = spacing
    ...
    # ZY面 (行=Z, 列=Y) => aspect = dz / dy = 0.0362 / 0.2 = 0.181
    # ZX面 (行=Z, 列=X) => aspect = dz / dx = 0.0362 / 0.2 = 0.181
    
    views = [
        (ext[cz, :, :], inp[cz, :, :], fak[cz, :, :], tgt[cz, :, :], dy/dx, "XY (Axial)\nZ_center"),
        (ext[:, :, cx], inp[:, :, cx], fak[:, :, cx], tgt[:, :, cx], dz/dy, "ZY (Sagittal)\n X_center"),
        (ext[:, cy, :], inp[:, cy, :], fak[:, cy, :], tgt[:, cy, :], dz/dx, "ZX (Coronal)\n Y_center")
    ]
    
    # 你可以根据实际代表的含义修改这里的 Title
    titles = ["Extra (Ref)", "Input (LQ)", "Fake (Generated)", "Target (HQ)"]
    
    for row in range(3):
        slice_ext, slice_inp, slice_fak, slice_tgt, aspect_ratio, row_label = views[row]
        ...
        if row == 0: ax.set_title(titles[3], fontsize=14, fontweight='bold')
        ax.set_xticks([]); ax.set_yticks([])
    ...
    plt.close(fig)
# def log_orthogonal_views_to_tb(writer, global_step, input_t, fake_t, target_t, spacing=(0.0362, 0.2, 0.2), save_dir=None):
    """
