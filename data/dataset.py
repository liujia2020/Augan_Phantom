...
class UltrasoundDataset(data.Dataset):
    ...
    def _read_volume(self, path):
        ...
        v_min = data.min()
        v_max = data.max()
        # 将整个体数据从其原生范围 (比如 -60~0) 线性映射到 [-1.0, 1.0]
        if v_max - v_min > 1e-6:
            data = (data - v_min) / (v_max - v_min) * 2.0 - 1.0
        else:
            data = np.zeros_like(data) # 防止除零异常
    ...
    def __getitem__(self, index):
        ...
        # 3. 边界 Padding (如果遇到比 patch_size 还小的数据)
        # 注意：现在背景填充值使用 -1.0 (纯黑)
